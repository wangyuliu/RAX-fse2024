...
class ModelTrainer:
    ...
    def save(self):
            # 保存模型 an-cc的模型 ak--无cc的模型 as--划分成0.1测试集 同时用cc bb-有cc的109分数据集 t无cc的109
        #     b 缺少了builtin
        #     目前用的bb+ak
        #     bc -new have cc
        dump(self.rf, './data/models/rf_model_bs.joblib')
        dump(self.ada, './data/models/ada_model_bs.joblib')
        dump(self.svm, './data/models/svm_model_bs.joblib')
        dump(self.xgb, './data/models/xgb_model_bs.joblib')
    def train(self):
        # 训练模型
...
if __name__ == '__main__':
    param_list = [1000, 10, 6, 6, 'poly', 6, 5.8, 7.3, 'DecisionTreeClassifier', 0.01, 1000, 1000, 30, 0.01, 3]
