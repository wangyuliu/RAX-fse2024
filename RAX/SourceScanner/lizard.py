...
class OutputScheme(object):
    ...
    @staticmethod
    def _head(captions):
        return "\n".join(("=" * len(captions), captions, "-0-" * len(captions)))
...
def print_and_save_modules(all_fileinfos, scheme):
    saved_fileinfos = []
    print(scheme.function_info_head()+"093042930492395532314235467587653524678576435246")
    odd=0
    ...
    print("==============================================================")
    print("NLOC chengrongji" + scheme.average_captions() + " function_cnt    file")
    print("--------------------------------------------------------------")
    ...
    ok = str(oddss)
    okk = "CC"
    # you should set address of file here
    with open("D:/tool/RAX/SourceScanner/data/result/1.csv", 'a', encoding='utf-8') as f:
        f.write(
            f"{okk},{ok}\n")
...
if __name__ == "__main__":
    # method to get porting complexity vector
    # you should set address of project in ./missions.py and ./settings.py
    ...
    # you should set address of project in Run-Edit Configurations
    # and set in 836 line
    main()
